...
def tcp_link(sock, address):
    ...
    # latest report
    report_file = sorted(
        [(x, x.stat().st_mtime) for x in os.scandir(REPORT_DIR) if x.is_file() and x.name.endswith(".html")],
        key=lambda i: i[1])[-1][0]

    file_name, file_path = report_file.name, report_file.path
    file_name_len, file_size, file_md5 = len(file_name), os.path.getsize(file_path), calc_md5(file_path)

    # send report
    print("\nSending report...")
    file_head = struct.pack(
        HEAD_STRUCT, file_name.encode('utf-8'), file_name_len, file_size, file_md5.encode('utf-8'))
    sock.send(file_head)
    with open(file_path, 'rb') as fr:
            for msg in fr:
                sock.send(msg)
